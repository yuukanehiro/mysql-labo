...
type DemoDeadlock struct {
	...
	CompanyID int    `gorm:"index:idx_company_user,unique"`
	UserID    int    `gorm:"index:idx_company_user,unique"`
	Data      string
}
...
func insertBatch(db *gorm.DB, companyID int) error {
	return db.Transaction(func(tx *gorm.DB) error {
		...
		for batch := 0; batch < 20; batch++ {
			...
			for i := 0; i < 1000; i++ {
				// 例
				id := fmt.Sprintf("Z%08d_%02d", rand.Intn(20000), companyID) // 故意にバラけさせる
